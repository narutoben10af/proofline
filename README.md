...
A disabled-by-default Supabase persistence seam and owner-RLS migrations are available for
review. They do not change the process-local default or claim production readiness; see the
[guarded setup path](docs/supabase-persistence-setup.md). The authenticated Magic Assistant uses
only a numeric-free owner evidence index; authoritative values remain in the reviewed analysis.
